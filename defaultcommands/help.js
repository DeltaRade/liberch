...
class HelpCommand extends Command {
	...
	execute(client, message, args) {
		...
		if(!args.length) {
			...
			data.push(`\nYou can use \`${client.prefixes.replace('\\\\', '')}help [command name]\` to get info on a specific command!`);
		...
		if (command.usage) data.push(`**Usage:** ${client.prefixes.replace('\\\\', '')}${command.name} ${command.usage}`);
